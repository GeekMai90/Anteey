<template>
  <div class="writing-view">
      ...
      <div class="topToolBar">
        <div class="topToolBar-header">
          ...
          <div class="topToolBar-right">
            <!-- 新增文章按钮 -->
            <div class="add-article-button" @click="addArticle">
              <div class="icon">
                <Plus
                  theme="outline"
                  size="18"
    ...
    <div class="writing-view-container">
...
import { SortTwo, Write, Plus } from '@icon-park/vue-next'
import { useWritingStore } from '@renderer/stores/writingStore'
import type { Article, ArticleStatus } from '@shared/types/writing'
import { formatFullDate } from '@renderer/utils/dateUtils'
...
const fetchArticles = async () => {
  await writingStore.fetchArticles()
}

// 排序后的文章列表
const sortedArticles = computed(() => {
  const articles = writingStore.articles
...
const addArticle = async () => {
  try {
    const newArticle = await writingStore.createArticle({
      title: '新文章'
    })
    // 创建成功后直接进入编辑页面，使用新的路由名称
    router.push({
      name: 'WritingDetail',
...
// 复用 EdWhiteboardView.vue 的基础样式结构
.writing-view {
  height: 100vh;
  display: flex;
...
// ... 其他样式保持与 EdWhiteboardView.vue 一致 ...
</style>
