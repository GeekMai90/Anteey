      ...
      <div class="topToolBar">
        <div class="topToolBar-header">
          <div class="topToolBar-left">
            <div class="icon">
              <Bookshelf theme="outline" size="20" fill="var(--color-yellow)" :strokeWidth="3" />
            </div>
...
import { SortTwo, Search, Close, Bookshelf } from '@icon-park/vue-next'
...
const displayedNotes = computed(() => {
  ...
  return searchResults.value.filter((note) => note.cardType === 'Bibcard')
})
...
eventBusNoteRestored.on(() => {
  console.log('BibCard.vue→ 监听到笔记从回收站恢复事件')
  resetPagination()
  fetchNotes()
})

eventBusEmptyNotesMovedToTrash.on(() => {
  console.log('BibCard.vue→ 监听到空笔记移到回收站事件')
  resetPagination()
  fetchNotes()
})
