...
import { useEventBus, useMemoize, useThrottleFn } from '@vueuse/core'
...
const pageSize = ref(15) // 每页显示的笔记数量

const allFilteredNotes = ref<Note[]>([])
  ...
  (
    ...
    return notes
      .filter((note: Note) => {
        ...
        } else if (selectedCardBox && selectedCardBox.id !== '0000') {
          return note.cardBoxId === selectedCardBox.id
        }
        return true
      })
      .filter((note: Note) => {
        return selectedCardTypes.length === 0 || selectedCardTypes.includes(note.cardType)
      })
      .sort((a: Note, b: Note) => {
        let comparison = 0
        switch (currentSort) {
...
watchEffect(() => {
  allFilteredNotes.value = memoizedFilter(
    allNotes.value,
    isInboxSelected.value,
    selectedCardBox.value,
...
const displayedNotes = computed(() => {
  ...
  return allFilteredNotes.value.slice(0, endIndex)
})
...
const checkScroll = useThrottleFn(() => {
  ...
  // 当距离底部 200px 时开始加载更多
  if (scrollHeight - scrollTop - clientHeight < 1000 && !isLoading.value) {
    console.log('接近底部，触发加载更多')
    loadMoreNotes()
  }
}, 200)
...
const loadMoreNotes = async () => {
  try {
    if (currentPage.value < totalPages.value && !isLoading.value) {
      console.log('加载更多笔记，当前页码:', currentPage.value)
      // isLoading.value = true
      currentPage.value++
      // 模拟异步加载，实际应替换为真实的API请求
      await new Promise((resolve) => setTimeout(resolve, 500))
      console.log('完成加载更多笔记，当前页码:', currentPage.value)
      isLoading.value = false
    }
  } catch (error) {
    console.error('加载更多笔记时发生错误:', error)
    isLoading.value = false
  }
}
